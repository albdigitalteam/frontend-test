...
const PostCard = ({ postlist = [] }) => {
  const [itemOpen, setItemOpen] = useState({index:'', cmt: []})

  const handleShowComments = (ix, commentList, isOpen) => {
    if(!isOpen) return setItemOpen({ index: ix, cmt: commentList })
    setItemOpen({ index: '', cmt: [] })
  }
    ...
    return (
      <ViewWrapper key={ix}>
        <Wrapper>
          ...
          <WrapperProfile>
            <ButtonBox see>
              <Button
                ...
                width={49}
                onPress={() => {
                handleShowComments(ix, post.commentList, isOpen)
              }}>
          ...
          {isOpen && handleComments(itemOpen.cmt)}
        </Wrapper>
      </ViewWrapper>
    )
  })
